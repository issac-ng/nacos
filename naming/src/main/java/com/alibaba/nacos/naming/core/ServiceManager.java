...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
            throws NacosException {
        //构建实例 key= com.alibaba.nacos.naming.iplist.ephemeral.demo##DEFAULT_GROUP@@nacos.test.1
    ...
    private void putServiceAndInit(Service service) throws NacosException {
        //把service存放在存储到serviceMap中
        putService(service);
        //服务初始化
        service.init();
        /**
         * consistencyService添加监听器 Listener 即当前 com.alibaba.nacos.naming.core.Service
         */
        consistencyService
                .listen(KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), true), service);//service就是一个RecordListener
        consistencyService
